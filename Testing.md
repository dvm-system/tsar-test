...
- `env::` - группа переменных, устанавливающая занчения переменных окружения перед запуском теста,
например `env::DVMH_NUM_CUDAS=1` установит значение переменной окружения `DVMH_NUM_CUDAS` для запуска DVMH программ, соответсвующей данному тесту, на графическом ускорителе.
...
5. В зависимости от режима выполнение теста выполняется сравнение полученных результатов (режим `check`) с эталонными или их копирование в директорию с эталонными результатами (режим `init`). Чтобы задать имя директории с эталонноыми результатами можно использовать переменную `$sample_dir`, имеющую значение `sample` по умолчанию. Например, можно создать директорию, имя которой зависит от используемой платформы для запуска тестов (`sample_dir=sample_$platform`). Сравниваемые (копируемые) результаты (файлы) должны быть указаны в одной из следующих переменных внутри конфигурационного файла:
    ...
    - `sample_map` - список пар имен сравниваемых (копируемых) файлов; каждая пара содержит имя одного из файлов, порождаемого результатом запуска теста, и имя соответсвующего эталонного файла. Имя файла с результатом указывается относительно директории с результатами, а имя файла эталона относительно `$samlple_dir`.
...
В случае выполнения теста в режиме `init` на первом шаге в директорию с результатами запуска будут скопированы все исходные файлы, в том числе и заголовочные файлы, с сохранением иерархии директорий. Затем в этой директории будет выполнена команда `$run`, при этом путь до инструмента TSAR будет взят из переменной `$tsar`, которая будет задана утилитой `sapfor-test`. В результате в директории с результатами запуска исходный код последовательной программы будет модифицирован и будет сгенерирована параллельная версия программы с соответсвующим изменением исходных файлов. Кроме того результаты запуска инструмента TSAR, выданные на стандартные потоки вывода и ошибок, будут записаны в файл `output.txt`, указанный в комнде `$run` (`>output.txt 2>&1`). Затем параллельная версия программы, а также файл `output.txt` будут скопированы в директорию `sample`. В случае выполнения теста в режиме `check`, вместо копирования будет выполнено сравнение указанных файлов с соответствующими файлами в директории `sample`.

Важно иметь ввиду, что если какие-то исходные файлы не были модфицированы (например заголовочные файлы), то их рекомендуется копировать в директорию `sample` в любом случае (указывая их в переменной `$sample`). Это позволит избежать ситуации, когда при последующих изменения в инструменте TSAR эти файлы окажутся модифицируемыми, но запуск теста не позволит выявить изменение в поведении инструмента (если данные файлы отсутствуют в переменной `$sample`).

Если на разных платформах инструмент TSAR ведет себя по-разному, например, из-за особенностей построения вунтреннего представления LLVM IR для распараллеливаемых программ, то можно исползовать переменную `sample_dir=sample_$platform`, что создавать разные эталонные результаты на разных платформах. Стоит отметить, что перед выполнением теста в режиме проверки (`check`) на некоторой платформе необходимо убедится, что данный тест был инициализирован на данной платфомре, то есть что существует соответсвующая директория с эталонными результатами.
...
В результате выполнения данного теста будут использованы исходные файлы DVMH-программы, уже расположенные в директории с результатами теста (после выполнения теста заданного файлом `tsar.conf`). Так как переменная `$copy` не задана копирование дополнительных файлов в эту директорию не выполняется. В результате выполнения данного теста будут созданы два файла: `compile.txt`, содержащий выдачу в стандартный поток вывода и ошибок, и исполняемый файл с платформо зависимым расширением, задаваемым опцией `$exe_extension`. При этом в качестве результата данного теста, который будет скопирован в директорию `sample` (для которого будет выполнено сравнение с соответсвующим файлом из директории `sample`), рассматривается только файл `compile.txt`.
...
В данном случае конфигурация запуска DVMH-программы определяется тем, как были заданы переменные окружения (группа `env::`), а также переменной `$gird`, которая является параметром данного теста и указывается при запуске теста. При этом имя файла (переменная `$output`) с результатами запуска зависит от используемой конфигурации запуска, в то время как эталонный результат запуска содержится в файле `run.txt`, одинаковом для всех запусков теста (переменная `$sample_map`).
...
Рассмотрим более продробно структуру тестового набора на примере тестового набора `check`, остальные тестовые наборы устроены аналогичным образом.
...
Внутри второй параллельной области также выделена группа последовательно выполняющихся тестов: это тесты которые используют для своего выполнения графический ускоритель. Чтобы несколько тестов одновременно не занимали один и тот же ускоритель, тесты запускаются последовательно. При этом параллельно с этими запусками выполняются запуски на ядрах центрального процессора. Так как запуски на центральном процессоре не расположены внутри параллельной области, то они выполняются параллельно независимо от того сколько физически доступно ядер. Это допустимо так как данные тесты не являются тестами прозиводительности, а проверяют корректность генерируемых программ.
...
Затем выполняется группы тестов отвечающих за запуск, при этом параллельно выполняется 2 запуска теста dvmh_sm_1/run и два запуска теста dvmh_sm_2/run на мультипроцессоре. Одновременно с этим выполняются тесты на грфическом процессоре, при этом в каждый момент времени на графическом процессоре выполняется только один тест.
...
3. Добавить правила запуска теста и идентификатор в файлы `init` и `check` (или `fail`).
...
Опция `-I` указывает директорию где расположены плагины, используюемые для запуска отдельного теста. Плагин - это скрипт на языке Perl, который вызывается системой PTS для выполнения теста, плагин выбирается в соответсвии со значением переменной `$plugin` внутри конфигурациооного файла для теста.
