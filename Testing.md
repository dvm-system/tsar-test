...
- `env::` - группа переменных, устанавливающая занчения переменных окружения перед запуском теста, например `env::DVMH_NUM_CUDAS=1` установит значение переменной окружения `DVMH_NUM_CUDAS` для запуска DVMH программы, соответсвующей данному тесту, на графическом ускорителе.
