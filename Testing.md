...
- `sys_dir` - путь до системной директории внутри используемого репозитория с тестами (обычно это директория `sys` в корне репозитория), например, в данной директории могу быть расположены вспомогательные файлы, необходимые для запуска тестов; в качестве примера можно рассмотреть скрипт `output_filter.pl`, входящий в состав репозитория [tsar-test](https://github.com/dvm-system/tsar-test) и удаляющий из переданного ему на вход файла платформа зависимые пути.
...
run = "$dvm c $sources $options >compile.txt 2>&1 | $perl -p $sys_dir/output_dvm_filter.pl >compile.txt"
```

Фильтр `output_dvm_filter.pl` используемый для постобработки информации, генерируемой компилятором DVM системы, удаляет абсолютные пути, заменяя их на пути относительно директории установки DVM системы или текущей директории запуска компилятора, кроме того он удалет завершающий символ `\n`, который добавляет DVM система при запуске программ.
...
clean = $output sts.gz+
run = "$dvm run $grid main${exe_extension} $grid 2>&1 | $perl -p $sys_dir/output_dvm_filter.pl >'$output.txt'"
```
